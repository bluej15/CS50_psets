...
void initBricks(GWindow window)
{
    int brick_place_y = 50;
   
    for (int i = 0; i < ROWS; i++, brick_place_y += 13)
    {   
        int brick_place_x = 5;
        for (int j = 0; j < COLS; j++, brick_place_x += 39)
        {
            GRect brick = newGRect(brick_place_x, brick_place_y, 36, 10);
            setColor(brick, "RED");
...
GOval initBall(GWindow window)
{
    GOval ball = newGOval (190, 350, 20, 20);
    setColor(ball, "BLACK");
    setFilled(ball, true);
    add(window, ball);
    return ball;
